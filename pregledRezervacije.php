...
<br><br>
      ...
      $upitZaRezervacije = "SELECT * FROM rezervacije WHERE korisnickoIme='" . $_SESSION['korisnik'] . "' and nazivModela='".$_GET['nazivModela']."'";

      $rezultat=mysqli_fetch_assoc(mysqli_query($konekcija,$upitZaRezervacije));

      echo "<h2>".$rezultat['nazivModela']."</h2>";
      echo "<img src='".$rezultat['slikaModela']."' style='width:100%; height: auto;'>"."<br><br>";
      echo "<h4> Cena modela: ".$rezultat['cena']."</h4>";
      echo "<h4> Datum rezervacije: ".$rezultat['datumRezervacije']."</h4><br><br>";
      echo "<button class='sub'><a class='sub' style='text-decoration:none; color: white;' href='pregledRezervacije.php?otkazi=" . $_GET['nazivModela'] . "&vremeRezervacije=" . $rezultat['datumRezervacije'] . "' >Otkaži rezervaciju</a></button>";


      mysqli_close($konekcija);
      
    }
    else if(isset($_GET['otkazi']) and isset($_GET['vremeRezervacije']))
    {
            $konekcija = mysqli_connect("sql306.epizy.com", "epiz_28113732", "RtEZ630f1aYz", "epiz_28113732_w316");
            // Test da li se povezao sa bazom
            if (mysqli_connect_errno())
            ...
            $upitZaOtkazivanje = "DELETE FROM rezervacije WHERE korisnickoIme= '".$_SESSION['korisnik']."' and nazivModela='".$_GET['otkazi']."' and datumRezervacije='" . $_GET['vremeRezervacije'] . "'" ;
           if(mysqli_query($konekcija, $upitZaOtkazivanje)){
             echo "<script>alert('Uspesno ste otkazali rezervaciju.')</script>";
            echo("<script>location.href = '/BMW/index.php';</script>");
          }
            mysqli_close($konekcija);
}
      ?>
