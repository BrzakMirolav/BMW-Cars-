...
<html lang="en">
  <head>
    ...
    <title>BMW - Pregled Rezervacija</title>
          ...
          <a class="nav-link" href="modeli.php">Modeli</a>
                ...
                <!-- M Logo -->
                 ...
                 if(isset($_SESSION['korisnik']))
                 {
                    ...
                    echo "<a class='nav-link dropdown-toggle active' data-toggle='dropdown' href='#' role='button' aria-haspopup='true' aria-expanded='false'>"
...
<br><br>
      ...
      <div class="container-fluid" style="width: 100%; padding: 10px;">  <!--Pocetak Body kontejnera-->               
        ...
        $upitZaListanjeRezervacija = "SELECT * FROM rezervacije";
        echo "<select name='listaRezervacija' id='listaRezervacija'>
                <option value='0'>--- Izaberite rezervaciju ---</option>";
        if($rezultat = mysqli_query($konekcija, $upitZaListanjeRezervacija)) {
            // Citanje reda pojedinacno
            
            while ($row = mysqli_fetch_assoc($rezultat)) {
                echo "<option value='korisnickoIme=". $row['korisnickoIme'] . "&nazivModela=" . $row['nazivModela'] . "&datumRezervacije=" . $row['datumRezervacije'] . "'>" . $row['korisnickoIme']." | ". $row['nazivModela']." | ". $row['datumRezervacije']."</option>";
        ...
        <br><br>
        <button id="brisanjeRezervacije" class="sub"> Obriši rezervaciju</button>
   ...
   if(isset($_GET['korisnickoIme']) and isset($_GET['nazivModela']) and isset($_GET['datumRezervacije']))
    {
            $konekcija = mysqli_connect("sql306.epizy.com", "epiz_28113732", "RtEZ630f1aYz", "epiz_28113732_w316");
            // Test da li se povezao sa bazom
            if (mysqli_connect_errno())
            ...
            $upitZaOtkazivanje = "DELETE FROM rezervacije WHERE korisnickoIme= '".$_GET['korisnickoIme']."' and nazivModela='".$_GET['nazivModela']."' and datumRezervacije='" . $_GET['datumRezervacije'] . "'" ;
           if(mysqli_query($konekcija, $upitZaOtkazivanje)){
           echo "<script>alert('Uspešno si obrisao rezervaciju');</script>";
           echo("<script>location.href = '/BMW/otkazivanjeRezervacijaAdmin.php';</script>");
           }
            mysqli_close($konekcija);
}
      ?>
...
<script>
$("#brisanjeRezervacije").click(function() {
    let rezervacija = $("#listaRezervacija :selected").val(); 
    window.location = 'otkazivanjeRezervacijaAdmin.php?' + rezervacija;
});
</script>