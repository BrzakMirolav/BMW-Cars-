...
<body>
  ...
  <div class="container-fluid" style="width: 100%; padding:10px; ">
    <br>
    <h3 >Pronađite svoj BMW</h3>
    <input style='width: 30%;' type="text" id="search" onkeyup="myFunction()" placeholder="Pretražite">
    <h1 style="text-align:center;">Svi modeli</h1>


    <div class="container-fluid model" >
      <!--Model kontejner pocetak-->
      <ul id='myUL'> <!--POCETAK UL ZA PRETRAGU-->
        <?php
        $konekcija = mysqli_connect("sql306.epizy.com", "epiz_28113732", "RtEZ630f1aYz", "epiz_28113732_w316");
        // Test da li se povezao sa bazom
        if (mysqli_connect_errno())
        ...
        $upitZaVadjenjeSlika = "SELECT slikaModela, nazivModela FROM slike";
        if($rezultat = mysqli_query($konekcija, $upitZaVadjenjeSlika)) {
            // Citanje reda pojedinacno
            while ($row = mysqli_fetch_row($rezultat)) {
                echo "
                <div class='row'>
                    <div class='col-md-12'>
                    <form action='prikazModela.php' method='POST'>
                     <input type='hidden' name='nazivModela' value='$row[1]'/>
                     <li><a href='#' class='odabranModel' style='color:transparent'><img style='width: 25%; height: auto;' class='slikeModela' src='$row[0]' alt='$row[1]'>$row[1]</a></li>
                        </form>
                    </div>
                </div>";
            }
        ...
        // Zatvaranje konekcije za bazom
        mysqli_close($konekcija);

        ?>
         </ul>  <!--KRAJ UL ZA PRETRAGU-->
    </div>
    <!--Model kontejner kraj-->


</div>    <!--Body kontejner kraj-->
    <br><br><br>
...
<script>
// Skripta pomocu koje se formira sablon stranica za model odabran
$('.odabranModel').click(function(e) {
   e.preventDefault();
