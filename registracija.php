...
<head>
  ...
  <div class="container-fluid" style="width: 100%;">
    ...
    <div class="toggle"> <!--Pocetak toggle-->
      ...
      <li class="nav-item ">
        <a class="nav-link active" href="registracija.php">Registruj se</a>
      </li>
...
<body>
  <br>
  <div class="container-fluid banner" style="width: 100%;" >
    <!--Pocetak Body kontejnera-->
    

    <div class="reg" style="background: rgb(55,177,251); background: linear-gradient(124deg, rgba(55,177,251,1) 0%, rgba(1,27,219,1) 50%, rgba(255,0,0,1) 100%);">
      <!--Pocetak registracionog diva-->
      <br>
      ...
      <form action="registracija.php" method="post">
        ...
        <input type="text" id="kime" name="kime" placeholder="Unesite vaše korisničko ime" required> <br>
        ...
        <input type="submit" value="Registruj se" id="sub" class="sub"> <br><br>

        Već imate nalog?<a href="prijava.php" style="text-decoration:none;"> Prijavi se</a> <br> <br>

      </form>

    </div>
    <!--Kraj registracionog diva-->
    ...
    if(isset($_POST["kime"]) and $_POST["kime"] != "" and
      ...
      (isset($_POST["lozinka"]) and $_POST["lozinka"] != "") and
      (isset($_POST["email"]) and $_POST["email"] != ""))
    {
      ...
      $vreme = date('m-d-Y');
        ...
        if ($kimeQuery = mysqli_query($konekcija, "SELECT korisnickoIme FROM korisnici WHERE korisnickoIme = '$kime'"))
        { 
        $row = mysqli_fetch_row($kimeQuery);
        if($row[0] == $kime)
         echo "<script>alert('Korisnik vec postoji u bazi');</script>"; 
        
          else
          {
        //Unos korisnika u bazu
        
        if (mysqli_query($konekcija, "INSERT INTO korisnici VALUES (null, '$kime', '$lozinka', '$ime', '$prezime', '$email', DEFAULT, 'Korisnik', DEFAULT)"))
        echo "<script>alert('Uspešno ste kreirali nalog!'); 
